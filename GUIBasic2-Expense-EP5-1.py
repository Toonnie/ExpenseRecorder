...
GUI = Tk()
GUI.title('โปรแกรมบันทึกค่าใช้จ่าย by Toonnie')
GUI.geometry('500x650+800+20')
...
#file menu  
filemenu = Menu(menubar,tearoff=0) #(menubar,tearoff=0) เพื่อให้ไม่สามารถดึงออกมาได้
menubar.add_cascade(label='File',menu=filemenu)
filemenu.add_command(label='Import CSV')
filemenu.add_command(label='Export to Googlesheet')
# Help
def About():
	messagebox.showinfo('About','สวัสดี โปรแกรมสำหรับบันทึกข้อมูลค่าใช้จ่าย\n สนใจติดต่อ 00000000')
...
icon_t1 = PhotoImage(file='expense.png').subsample(10) #  .subsample(2) = ย่อรูป
icon_t2 = PhotoImage(file='list.png').subsample(15)

Tab.add(T1, text=f'{"ค่าใช้จ่าย":^{5}}',image=icon_t1,compound='top')
Tab.add(T2, text=f'{"ค่าใช้จ่ายทั้งหมด":^{5}}',image=icon_t2,compound='top')
...
def Save(event=None):
	...
	try:
		...
		E1.focus()
	except exception as e: #ฟ้อง Error
		print('ERROR')
...
main_icon = PhotoImage(file='pp.png').subsample(50)
...
icon_b1 = PhotoImage(file='save.png').subsample(10)

B2 = ttk.Button(F1,text=f'{"Save": >{10}}',image=icon_b1,compound='left',command=Save)
B2.pack(ipadx=5,ipady=5,pady=5)
...
L = ttk.Label(T2,text='ตารางแสดงผลลัพท์ทั้งหมด',font=FONT1).pack(pady=5)
